...
const saveDoc = async (collection, state) => {
  ...
  if (id) {
    ...
    await firebase.firestore().collection(collection).doc(id).set(doc);
    ...
    const result = await firebase.firestore().collection(collection).add(doc);
...
const readDoc = async (collection, id) => {
  //read a document from database and return it
  const docRef = await firebase.firestore().collection(collection).doc(id).get();
